...
class Manning_Score_fm : Fragment() {
    ...
    private fun initAdaptor() {
        val mainAdapter = MedCalListAdapter(getMainData(), "32")
        val appendixAdapter = MedCalListAdapter(getAppendixData(), "20")
    ...
    private fun getMainData() = mutableListOf(
        arrayOf("评分", "胎儿情况", "处理原则"),
        arrayOf("10分", "无急、慢性缺氧", "每周复查1次，高危每周2次"),
        arrayOf("8分", "缺氧可能性小", "每周1次，高危每周2次，羊水过少可终止"),
        arrayOf("6分", "可疑缺氧", "24h内复查，仍≤6或羊水过少终止"),
        arrayOf("4分", "可有缺氧", "24h内复查，仍≤6或羊水过少终止"),
        arrayOf("2分", "急性或伴慢性缺氧", "胎肺成熟终止；不成熟激素治疗48h内终止"),
        arrayOf("0分", "急、慢性缺氧", "终止妊娠，胎肺不成熟同时激素治疗")
    )

    private fun getAppendixData() = mutableListOf(
        arrayOf("相关解释", "5项指标：无应激实验(NST)、胎儿呼吸样运动(FBM)、胎动(FM)、胎儿肌张力(FT)及羊水容量(AFV)。"),
